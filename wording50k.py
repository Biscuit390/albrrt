...
def writesentence(word,wordcount):
    word = word.lower()
    writing = []
    writing.append(word)
    #wtw = randint(1,35)
    for I in range(0,int(wordcount)-1):
        gotword = False
        try:
...
def randomthought(count):
    try:
        return writesentence(choice(Starters),count)
    except IndexError:
        return writesentence(choice(Recents),count)


def passcensor(words):
    didpass = True
    Bad = ['fuck','porn','yiff','pee','defecate','bitch','anal','douche','shit','gay','daddy','dick','tit','cock','cis','cunt','sjw','cum','sex','dildo','penis','nigg','bugger']
    for x in Bad:
        if x in words.lower():
            didpass = False
            #print("A bad word")
